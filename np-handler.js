...
export async function handleNovaPoshta(req, res) {
  ...
  try {
    ...
    // Опис відправлення: чистимо від емодзі/диких символів і обрізаємо
    const rawItemsDescription =
      order.line_items?.map((i) => i.name).join(", ") ||
      `Shopify order ${order.name}`;

    let description = rawItemsDescription
      // прибираємо все, що не літера/цифра/пробіл/.,- (ASCII + кирилиця частково ок)
      .replace(/[^0-9A-Za-zА-Яа-яІіЇїЄєҐґ\s.,-]/g, "")
      .trim();
    ...
    // позначаємо це замовлення як вже оброблене
